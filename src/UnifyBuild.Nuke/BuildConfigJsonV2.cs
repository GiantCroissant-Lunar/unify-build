...
/// <summary>
/// Generic build configuration schema v2 - replaces domain-specific terminology
/// (HostsDir, PluginsDir, ContractsDir) with flexible project groups.
/// </summary>
...
/// <summary>
/// Loader for v2 build configuration with automatic schema version detection.
/// </summary>
public static class BuildContextLoader
{
    /// <summary>
    /// Load build configuration from JSON file, automatically detecting v1 or v2 schema.
    /// </summary>
    ...
    public static BuildContext FromJson(AbsolutePath repoRoot, string configFile = "build.config.json")
    {
        ...
        if (!json.Contains("\"projectGroups\"", StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException(
                ...
                + "v1 is no longer supported. Migrate to v2 schema as described in docs/rfcs/rfc-0001-generic-build-schema.md");
        }

        Serilog.Log.Information("Detected v2 build configuration schema");
        return LoadV2Config(repoRoot, json, options);
    }

    private static BuildContext LoadV2Config(AbsolutePath repoRoot, string json, JsonSerializerOptions options)
    {
        var cfg = JsonSerializer.Deserialize<BuildJsonConfig>(json, options)
                  ?? throw new InvalidOperationException("Failed to parse v2 build config.");
            ...
            foreach (var (groupName, group) in cfg.ProjectGroups)
            {
                ...
                switch (group.Action.ToLowerInvariant())
                {
                    ...
                    case "compile":
                        ...
                        break;
                    default:
                        Serilog.Log.Warning($"Unknown action '{group.Action}' in group '{groupName}', treating as 'compile'");
                        compileProjects.AddRange(projectPaths);
                        break;
                }
            }
    ...
    private static List<string> DiscoverProjectsInGroup(AbsolutePath repoRoot, ProjectGroup group)
    {
        ...
        {
            Serilog.Log.Warning($"Source directory '{sourceDir}' does not exist, skipping group");
